...
const aboutSlides = Array.from(document.querySelectorAll('[data-about-slide]')).filter((slide) => slide.dataset.slideKey !== 'malena');
const aboutDots = Array.from(document.querySelectorAll('[data-about-dot]')).filter((dot) => dot.textContent.trim().toLowerCase() !== 'producción' && dot.textContent.trim().toLowerCase() !== 'production');
